...
class DashboardWhiteboard {
    ...
    static marker(
        ...
        x: number,
        y: number,
        strokeStyleNew: string,
        text: string
        ) {
        let strokeStyleOld = context.strokeStyle;
        context.beginPath();
    ...
    static indicateFourCorners(
        canvas: HTMLCanvasElement,
        context: CanvasRenderingContext2D
        ) {
        let x: number = canvas.clientLeft;
        let y: number = canvas.clientTop;
        let w: number = canvas.width; // clientWidth;
        let h: number = canvas.height; // clientHeight;
        ...
        context.fillText(
            "canvas.widthHeight:{" + canvas.width + "," + canvas.height + "}",
            x + 15,
            y + 35
            );
        context.fillText(
            "canvas.scroll:{" + canvas.scrollWidth + "," + canvas.scrollHeight + "}",
            x + 15,
            y + 55
            );
        context.fillText(
    ...
    canvasClickIndicator(event) {
        ...
        context.fillStyle = "black";
        context.fillText(
            "event.offset.X[" + event.offsetX + ",Y" + event.offsetY + "]",
            event.offsetX + 2,
            event.offsetY + 10
            );
        context.fillText(
            "event.screen.X[" + event.screenX + ",Y" + event.screenY + "]",
            event.offsetX + 2,
            event.offsetY + 30
            );
        context.stroke();
    ...
    canvasClickOld(event) {
        let canvas = (<HTMLCanvasElement>document.getElementById('myCanvas'));
        let context: CanvasRenderingContext2D = canvas.getContext('2d');

        // where is the mouse?
        let chapter: number = DashboardWhiteboard.hitTestChapterIndex(event.offsetX, event.offsetY);
    ...
    // tile coordinates
    public relativeTileLeft(): number {
        return 1;
    }
    public relativeTileTop(): number {
        return 1;
    }
    public relativeTileWidth(): number {
        return 1;
    }
    public relativeTileHeight(): number {
        return 1;
    }
    public relativeTileBorder(): number {
        return 10;
    }
    ...
    drawNormal(
        ...
        let chapterCount: number = 60;
        let chapterHeight: number = 40;
        ...
        for (let i: number = 1; i < chapterCount; i++) {
            let text: string = "0" + i;

            context.fillText(
                text.substr(text.length - 2, 2) + " - Kabler",
                40,
                chapterHeight * i
                );
        }
