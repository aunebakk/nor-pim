...
namespace norpim.DataAccessLayer {
    ...
    public class Product {
        ...
        public Product(Guid productId) {
            ...
            // product attributes
            var crudeAttribute = new CrudeProductAttributeData();
            ProductAttribute = crudeAttribute.FetchByProductId(productId);

            // product identifiers
            var crudeIdentifier = new CrudeProductIdentifierData();
            ProductIdentifier = crudeIdentifier.FetchByProductId(productId);

            // product information links
            var crudeInfo = new CrudeProductInfoData();
            ProductInfo = crudeInfo.FetchByProductId(productId);

            // product images
            var crudeImage = new CrudeProductImageData();
            ProductImage = crudeImage.FetchByProductId(productId);

            // product documentation
            var crudeDocumentation = new CrudeProductDocumentationData();
            ProductDocumentation = crudeDocumentation.FetchByProductId(productId);
        }
        ...
        public void Save(Guid userId) {
            ...
            using (var connection = new SqlConnection(ConfigurationManager.AppSettings["Conn"])) {
                ...
                try { 
                    ...
                    product.StateRcd = StateRef.Updated;
