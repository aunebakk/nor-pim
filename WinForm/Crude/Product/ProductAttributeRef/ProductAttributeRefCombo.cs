/*
  ...
  SQL2X Version: 1.0
  http://sql2x.org/
  Generated Date: 10/13/2020 2:15:20 PM
...
namespace SolutionNorSolutionPim.UserInterface {
    ...
    public partial class ProductAttributeRefCombo : UserControl {
        
        // Constructs the control with an empty ComboBox
        public ProductAttributeRefCombo() {
           InitializeComponent();
        }
        ...
        public void PopulateCombo() {
            if (!DesignMode && cboRef.DataSource == null) {
                CrudeProductAttributeRefServiceClient productAttributeRef = null;

                try {
                    productAttributeRef = new CrudeProductAttributeRefServiceClient();
                    List<CrudeProductAttributeRefContract> contracts = productAttributeRef.FetchAll();

                    cboRef.DataSource = contracts;
                    cboRef.DisplayMember = "ProductAttributeName";
                    cboRef.ValueMember = "ProductAttributeRcd";
                } catch (Exception ex) {
                    // MessageBox.Show(ex.Message);
                } finally {
                    if (productAttributeRef != null) productAttributeRef.Close();
                }
            }
        }
    }
