...
namespace SolutionNorSolutionPim.DataAccessLayer {
    ...
    public partial class DefaultTestOverviewData {
        ...
        public void Populate(IDataReader reader, DefaultTestOverviewDataOrdinals ordinals) {
            if (!reader.IsDBNull(ordinals.TestArea)) {
                TestArea = reader.GetString(ordinals.TestArea);
            }

            if (!reader.IsDBNull(ordinals.StartDateTime)) {
                StartDateTime = reader.GetDateTime(ordinals.StartDateTime);
            }

            if (!reader.IsDBNull(ordinals.DefaultTestRunResultRcd)) {
                DefaultTestRunResultRcd = reader.GetString(ordinals.DefaultTestRunResultRcd);
            }
        }
    }
